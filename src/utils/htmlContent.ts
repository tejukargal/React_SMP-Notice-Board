...
const linkifyUrls = (text: string): string => {
  ...
  return text.replace(urlPattern, (url) => {
    ...
    return `<a href="${href}" target="_blank" rel="noopener noreferrer"
      class="inline-flex items-center gap-1.5 px-3 py-1.5 mx-1 my-0.5 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-400 rounded-lg text-blue-700 font-semibold text-sm hover:from-blue-100 hover:to-indigo-100 hover:border-blue-600 hover:shadow-md transition-all duration-200 no-underline"
      style="text-decoration: none; display: inline-flex; align-items: center; white-space: nowrap; max-width: 300px; overflow: hidden; text-overflow: ellipsis;">
      ...
      <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${url.length > 40 ? url.substring(0, 40) + '...' : url}</span>
    </a>`
  })
}
